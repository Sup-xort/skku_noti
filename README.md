...
1. `scraper.py` 가 각 게시판을 파싱해 게시글의 `articleNo`·제목·카테고리·날짜·본문을 추출합니다.
   (서버 사이드 렌더링이라 JS 실행 불필요)
2. 게시판별 상태파일(`seen_*.json`)에 저장된 "이전에 본 글 번호"와 비교해 **새 글만** 골라냅니다.
   대학 공지는 새 글이 10건(한 페이지)을 넘겨 쌓여 있어도, 이미 본 글이 나올 때까지
   **다음 페이지로 넘어가며 전부** 수집합니다. (안전 상한 `MAX_PAGES`)
...
| `seen_articles.json` / `seen_sce.json` | 게시판별 본 글 번호 저장(자동 갱신) |
