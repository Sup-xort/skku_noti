...
2. 게시판별 상태파일(`seen_*.json`)에 저장된 기준선과 비교해 **새 글만** 골라냅니다.
   두 게시판 모두 글의 **고유 식별자로 URL 의 `articleNo`**(생성 시 부여되는 고정 DB id,
   재사용·이동 없음)를 사용합니다. `seen` 목록에 없는 `articleNo` 만 새 글입니다.
   - **대학 공지**: 목록을 최신 먼저 순서로 훑어, 새 글이 없는 페이지를 만나면 멈춥니다.
     한 시간에 10건(한 페이지)을 넘겨 쌓여 있어도 다음 페이지로 넘어가며 전부 수집합니다
     (안전 상한 `MAX_PAGES`). **고정공지("공지")는 교수임용 등 위주라 제외**합니다(`ignore_pinned`).
   - **학과 공지**: 로비에 노출되는 글을 `seen` 목록과 비교합니다.

> **왜 `articleNo` 로 식별하나?** 목록에 보이는 "No.XXXXX"는 현재 위치로 계산되는 번호라
> 글이 삭제되면 밀리지만, `articleNo` 는 글마다 영구히 박히는 고유 id 라 삭제·번호밀림·수정에
> 영향을 받지 않습니다. 그래서 "기준선 글이 삭제되고 새 글이 여러 개 올라와도" 전부 정확히
> 새 글로 잡히고, 글을 수정해도(같은 `articleNo`) 재알림이 가지 않습니다.

### 안전장치 (오작동/폭주 방지)
- `MAX_PAGES`: 페이지를 무한정 넘기지 않습니다.
- **같은 페이지 반복 감지**: `offset` 이 무시돼 동일 페이지가 반복되면 즉시 중단합니다.
- **실행 내 중복 제거**: 같은 글을 두 번 처리하지 않습니다.
- `MAX_NOTIFY`(기본 20): 상태 파일 손상 등으로 새 글이 폭증해도 **최신 N건만 전송**하되,
  본 글은 모두 `seen` 에 기록해 이후 재알림 폭주를 막습니다.
- 최초 실행 때 깊은 페이지까지 훑어 기준선에 담으므로, 새 글이 옛 글을 밀어내도 오탐이 없습니다.
...
| `seen_articles.json` / `seen_sce.json` | 게시판별 본 글 `articleNo` 목록(`seen`) 저장(자동 갱신) |
