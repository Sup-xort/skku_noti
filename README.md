...
2. 게시판별 상태파일(`seen_*.json`)에 저장된 기준선과 비교해 **새 글만** 골라냅니다.
   - **대학 공지**: 목록의 **표시번호(`No.XXXXX`, seq)**를 기준으로 판정합니다.
     ...
     **고정공지("공지")는 교수임용 등 위주라 알림에서 제외**합니다(`ignore_pinned`).
...
- `MAX_NOTIFY`(기본 20): 상태 파일 손상 등으로 새 글이 폭증해도 **최신 N건만 전송**하고
  기준선은 최댓값으로 전진시켜, 이후 재알림 폭주를 막습니다.

> **글 수정으로 순서가 꼬여도 안전한가?** 네. 판정 기준이 발행 순서인 표시번호(`No.`)라,
> `articleNo` 가 아무리 뒤섞여도 영향이 없습니다. 글을 수정해도 `No.` 는 보통 그대로라
> 재알림이 가지 않고, 혹시 수정으로 맨 위로 올라가(새 `No.` 부여) 다시 나타나면 1회만 알립니다.
